...
from settings import (
    BOREDOM_ACCW_THRESHOLD, BOREDOM_JITTER_EPSILON,
    COLOR_COLD, COLOR_FLOW, COLOR_WARM,
    FRUSTRATION_ACCW_THRESHOLD, FRUSTRATION_PF_THRESHOLD,
    TEMPO_BOREDOM, TEMPO_FLOW, TEMPO_FRUSTRATION,
)
...
class DDAController:
    ...
    def _classify(self, snapshot: EmotionSnapshot) -> EmotionState:
        if snapshot.accw < FRUSTRATION_ACCW_THRESHOLD or snapshot.frustration_risk > FRUSTRATION_PF_THRESHOLD:
    ...
    def _apply(self, state: EmotionState) -> None:
        ...
        if state == EmotionState.FRUSTRATION:
            self.engine.set_tempo(TEMPO_FRUSTRATION); self.engine.set_density(0.8)
            self.engine.set_theme(COLOR_COLD)
            self.audio.set_profile(AudioProfile.FRUSTRATION); self.audio.set_tempo(TEMPO_FRUSTRATION)
            return
        if state == EmotionState.BOREDOM:
            self.engine.set_tempo(TEMPO_BOREDOM); self.engine.set_density(1.2)
            self.engine.set_theme(COLOR_WARM)
            self.audio.set_profile(AudioProfile.BOREDOM); self.audio.set_tempo(TEMPO_BOREDOM)
            return
        self.engine.set_tempo(TEMPO_FLOW); self.engine.set_density(1.0)
        self.engine.set_theme(COLOR_FLOW)
        self.audio.set_profile(AudioProfile.FLOW); self.audio.set_tempo(TEMPO_FLOW)