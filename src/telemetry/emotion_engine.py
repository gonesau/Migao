"""Emotion inference core using windowed telemetry and NumPy."""
...
class EmotionEngine:
    ...
    def compute_accw(self, wtol_ms: float) -> float:
        p_errors = self._p_errors()
        if p_errors.size == 0:
            return 0.0
        return float(np.mean(np.maximum(0.0, 1.0 - (p_errors / wtol_ms))))

    def compute_jitter(self) -> float:
        p_errors = self._p_errors()
        if p_errors.size <= 1:
            return 0.0
        mean_p = np.mean(p_errors)
        variance = np.sum((p_errors - mean_p) ** 2) / (p_errors.size - 1)
        return float(np.sqrt(variance))

    def compute_frustration_risk(self, beta: float, gamma: float) -> float:
        exponent = -beta * (self.miss_streak - gamma)
        return float(1.0 / (1.0 + np.exp(exponent)))

    def snapshot(self, wtol_ms: float, beta: float, gamma: float) -> EmotionSnapshot:
        p_errors = self._p_errors()
        return EmotionSnapshot(
            accw=self.compute_accw(wtol_ms=wtol_ms),
            jitter=self.compute_jitter(),
            frustration_risk=self.compute_frustration_risk(beta=beta, gamma=gamma),
            miss_streak=self.miss_streak,
            sample_count=p_errors.size,
        )

    def _p_errors(self) -> np.ndarray:
        if not self.samples:
            return np.array([], dtype=float)
        return np.array([sample.p_i for sample in self.samples], dtype=float)
